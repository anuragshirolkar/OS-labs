...
#define PASS_PROGRAM "/" ROOT_PREFIX "/passVeri.exe"
...
static void Spawn_Init_Process(void) {
    int rc;
    struct Kernel_Thread *initProcess, *passP;

    int fail_count = 0;

    for (;fail_count < 3; fail_count++){
        rc = Spawn_Foreground(PASS_PROGRAM, PASS_PROGRAM, &passP);
        if (rc != 0){
          Print("Unable to start passVeri.exe\n");
          return;
        }
        else {
          int passSuccess = 0;
          passSuccess = Join(passP);
          if (passSuccess == 1) {
            Print("Password entered is wrong. Please re-enter.\n");
          }
          else if (passSuccess == -1){
            Print("Fatal error while Authenticating\nShutting Down\n");
            return;
          }
          else {
            Set_Current_Attr(ATTRIB(BLACK, GREEN | BRIGHT));
    ...
    if (fail_count >= 3) {
        Print("Incorrectly entered password three times in a row. Shutting Down\n");
        return;
    }
