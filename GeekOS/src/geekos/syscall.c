...
static int Sys_Open(struct Interrupt_State *state) {
    ...
    if (rc >= 0) {
        CURRENT_THREAD->file_open_count++;
...
static int Sys_CountCall(struct Interrupt_State *state) {
    return CURRENT_THREAD->sys_call_count;
}

static int Sys_FileOpenCount(struct Interrupt_State *state)
{
    return CURRENT_THREAD->file_open_count;
};

static int Sys_ReadSysCount(struct Interrupt_State *state)
{
    return CURRENT_THREAD->read_sys_count;
}

/*
* New get time of day
*/
static int Get_NewTOD(struct Interrupt_State *state
    __attribute__ ((unused))) {
//state->ebx[0] = 3;
    int temp = g_numTicks;
    if(!Copy_To_User(state->ebx, &temp, sizeof(int))) return -1;
    return 0;
}
